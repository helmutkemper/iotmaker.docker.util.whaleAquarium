...
func main() {
	...
	file += "package factoryContainerMongoDB\n\n"
	file += "type MongoDBVersionTag int\n\n"
	file += "const (\n"
	for k, line := range data {
		if k == 0 {
			file += fmt.Sprintf("\tKMongoDBVersionTag_%v MongoDBVersionTag = iota\n", replace(line))
		} else {
			file += fmt.Sprintf("\tKMongoDBVersionTag_%v\n", replace(line))
		}
	}
	file += ")\n\n"

	file += "func(el MongoDBVersionTag)String() string {\n" +
		"\treturn mongoDBVersionTags[el]" +
		"}\n\n"

	file += "var mongoDBVersionTags = [...]string{\n"
	file += fmt.Sprintf("\t\"%v\",\n", strings.Join(data, "\",\n\t\""))
	file += "}\n"

	ioutil.WriteFile("./typeMongoDBVersionTag.go", []byte(file), os.ModePerm)
}
